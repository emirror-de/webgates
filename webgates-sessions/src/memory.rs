...
//!
//! This module provides a framework-agnostic [`crate::repository::SessionRepository`]
...
use crate::lease::{LeaseAcquisition, RenewalLease};
...
use crate::tokens::{RefreshTokenHash, RefreshTokenHashRef};
...
impl SessionRepository for InMemorySessionRepository {
    async fn create_session(&self, input: CreateSession) -> RepositoryResult<()> {
        ...
        if guard.sessions.contains_key(&input.session.session_id) {
            return Err(crate::repository::RepositoryError::Conflict);
        }
        ...
        if family.subject_id != input.session.subject_id {
            return Err(crate::repository::RepositoryError::InvalidState);
        }
        ...
        match scope {
            ...
            RevokeSessionScope::SessionFamily => {
                let family_id = {
                    let guard = self.state.lock().await;
                    let Some(stored) = guard.sessions.get(&session_id) else {
                        return Err(crate::repository::RepositoryError::SessionNotFound);
                    };
    ...
    async fn revoke_family(&self, family_id: SessionFamilyId) -> RepositoryResult<()> {
        ...
        let Some(family) = guard.families.get_mut(&family_id) else {
            return Err(crate::repository::RepositoryError::SessionFamilyNotFound);
        };
        family.revoked = true;
    ...
    fn revoke_single_session(&mut self, session_id: SessionId) -> RepositoryResult<()> {
        self.revoke_single_session_state(session_id)
            .map_err(|error| match error {
                crate::repository::RepositoryError::SessionNotFound => {
                    crate::repository::RepositoryError::SessionNotFound
                }
                other => other,
            })
