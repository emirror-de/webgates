...
///   and `delete_account`.
/// - Implementations MUST enforce uniqueness of both `account_id` and `user_id`
///   at the storage layer where possible and SHOULD document timing
///   characteristics for lookups when used in authentication flows (to avoid
    ...
    G: Eq + Clone,
{
    ...
    ///
    /// Returns:
    /// - `Ok(())` when the repository is ready to use
