...
async fn surrealdb_permission_mapping_crud_and_queries() {
    ...
    let stored: Option<PermissionMapping> = match repo.store_mapping(mapping.clone()).await {
        Ok(s) => s,
    ...
    // 2) Query by ID
    let fetched_by_id: Option<PermissionMapping> = match repo.query_mapping_by_id(id).await {
        Ok(s) => s,
    ...
    let fetched_by_str: Option<PermissionMapping> =
    ...
    let all: Vec<PermissionMapping> = match repo.list_all_mappings().await {
        Ok(s) => s,
    ...
    let removed_by_str: Option<PermissionMapping> =
        match repo.remove_mapping_by_string("READ:API").await {
            Ok(s) => s,
    ...
    let removed_again: Option<PermissionMapping> =
        match repo.remove_mapping_by_string("read:api").await {
            Ok(s) => s,
    ...
    let stored2: Option<PermissionMapping> = match repo.store_mapping(mapping2.clone()).await {
        Ok(s) => s,
    ...
    // remove by id
    let removed_by_id: Option<PermissionMapping> = match repo.remove_mapping_by_id(id2).await {
        Ok(s) => s,
    ...
    let all_after: Vec<PermissionMapping> = match repo.list_all_mappings().await {
        Ok(s) => s,
...
async fn surrealdb_permission_mapping_uniqueness() {
    ...
    let stored1: Option<PermissionMapping> = match repo.store_mapping(m1.clone()).await {
        Ok(s) => s,
    ...
    let fetched_by_id = match repo.query_mapping_by_id(id1).await {
        Ok(s) => s,
