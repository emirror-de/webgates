//! OAuth2 errors for the framework-agnostic OAuth2 gate runtime.
//!
//! These error types model failures that can occur while configuring or executing the
//! OAuth2 Authorization Code + PKCE flow. They are transport-agnostic and can be mapped
//! by adapters (e.g., Axum) to appropriate HTTP responses.
//!
//! Design goals:
...
/// OAuth2-specific error type for the core OAuth2 gate runtime.
///
...
pub enum OAuth2Error {
    ...
    /// A required configuration field is missing on the OAuth2 gate builder.
    ...
    fn suggested_actions(&self) -> Vec<String> {
        match self {
            ...
            OAuth2Error::CookieTemplateInvalid { which, .. } => vec![format!(
                "Review {which} cookie template (SameSite/Secure/Max-Age). SameSite=None requires Secure=true"
            )],
